...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Initialize core components using SplitVec
    ...
    // Example: Create and trade F-NFTs with SplitVec for optimized fraction storage
    let mut fractions = SplitVec::new(); // Use SplitVec instead of standard Vec
    fractions.push(Fraction { owner: "Alice".to_string(), percentage: 50.0 });
    fractions.push(Fraction { owner: "Bob".to_string(), percentage: 50.0 });
    ...
    // DAO and Governance example with SplitVec for proposal and vote storage
    let mut proposal_options = SplitVec::new();
    proposal_options.push("Yes".to_string());
    proposal_options.push("No".to_string());

    let proposal_id = dao.create_proposal("Increase energy efficiency reward", "Alice".to_string(), proposal_options);
    ...
    // Fault detection and energy dashboard example with SplitVec for metrics
    let mut dashboard_metrics = SplitVec::new();
    dashboard_metrics.push(("total_energy".to_string(), blockchain.get_energy_metrics()));

    let mut dashboard = EnergyDashboard::new(dashboard_metrics);
    ...
    if blockchain.validate_contract_state("contract1") {
        let contract = SmartContract {
            ...
            state: SplitVec::new(), // Smart contract state uses SplitVec for state management
        };
    ...
    // Demonstrate creating a shard and adding a block to it using SplitVec
    blockchain.create_shard(1);
    let shard_block = Block::new(0, current_timestamp(), SplitVec::from(vec![tx.clone()]), "0".to_string());
